...
const Ex1 = () => {
  ...
  const [data, setData] = useState([
    ...
    { id: 2, user: '지민', title: '하이하이' },
    { id: 3, user: '희수', title: '멋쟁이' },
  ]);
  ...
  const [isSchText, setIsSchText] = useState(false);
  ...
  const evtInput = (e) => {
    ...
    schInput.current.classList.remove('req');
  };
  const evtAdd = (e) => {
    console.log(e);
  ...
  const search = () => {
    const newSchData = data.filter((item) => {
      ...
      if (searchText.trim().length) {
        ...
        setIsSchText(false);
        setMsg(nodataMsg[1]);
        schInput.current.classList.add('req');
        schInput.current.focus();
        return null;
      }
      return false;
      // return item
  ...
  return (
    <>
      ...
      {!isSchText && isResult && <span>{msg}</span>}
      <div>
        <table>
          <tbody>
            <tr>
