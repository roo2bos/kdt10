...
exports.getSignin2 = (req, res) => {
	console.log(req.body);
	res.render('signin2', { isSuccess: false });
};
...
// /user/signin 로그인 버튼(로그인 정보 전송)
exports.postSignin2 = (req, res) => {
	const { userid, pw } = req.body;
	console.log('postSignin2 : ', req.body); // form 인풋 값들. hong 1111

	User.postSignin2(req.body, (result) => {
		console.log('postSignin2 session: ', req.sessionID);
		if (result.length) res.send({ userInfo: result[0], isSuccess: true, session: req.session });
		else res.send({ isSuccess: false });
	});
};
...
exports.postProfile = (req, res) => {
	...
	User.postProfile(req.body, (result) => {
		...
		if (result.length)
			res.render('signin', { data: result[0], msg: `${result[0].name}님 환영합니다.`, isSuccess: true });
		// if(result.length) res.send({data:result[0],msg:`${result[0].name}님 환영합니다.`});
	});
};
exports.postProfile2 = (req, res) => {
	console.log('Cuser postProfile reuslt: ', req.body);
	User.postProfile2(req.body, (result) => {
		console.log('Cuser postProfile reuslt: ', req.body, result);
		// if(result.length) res.render('profile',{data:result[0]});
		if (result.length)
			res.render('signin2', { data: result[0], msg: `${result[0].name}님 환영합니다.`, isSuccess: true });
		// if(result.length) res.send({data:result[0],msg:`${result[0].name}님 환영합니다.`});
	});
};
...
exports.patchProfileUser2 = (req, res) => {
	User.patchProfileUser2(req.body, (result) => {
		res.send('회원정보 수정 성공!');
	});
};
...
exports.deleteProfileUser2 = (req, res) => {
	User.deleteProfileUser2(req.body.id, (result) => {
		// 모델에 rows를 result라는 변수명으로 받음.
		console.log('Cuser deleteProfileUser reuslt: ', req.body, result);
		res.send({ deleteUserId: req.body.id });
	});
